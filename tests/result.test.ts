...
describe('result function', () => {
  describe('basic functionality', () => {
    ...
    it('should handle Error instances', () => {
      ...
      const r = result<Error, Error>(error);
  ...
  describe('custom error detection', () => {
    ...
    it('should work with custom error detection', () => {
      ...
      const r = result<Error, ValidationError>(error, { isError: isValidationError });
      expect(r.isErr()).toBe(true);
      expect(r.unwrap()).toBe(error);
    });
  });
  ...
  describe('error type discrimination', () => {
    ...
    it('should handle single specific error type', () => {
      ...
      const r = result<Error, ValidationError>(error);
      expect(r.isErr()).toBe(true);
    ...
    it('should handle union of error types', () => {
      ...
      const r1 = result<Error, ValidationError | NetworkError>(getError('validation'));
      expect(r1.isErr()).toBe(true);
      expect(r1.unwrap()).toBeInstanceOf(ValidationError);

      const r2 = result<Error, ValidationError | NetworkError>(getError('network'));
    ...
    it('should handle custom error type guard with multiple types', () => {
      ...
      errors.forEach((error) => {
        const r = result<Error, AppError>(error, {
          isError: isAppError,
        });
        expect(r.isErr()).toBe(true);
        expect(r.unwrap()).toBe(error);
    ...
    it('should handle error type discrimination with tags', () => {
      ...
      const error = new ValidationError('invalid input');
      const r = result<Error, AppError, 'APP_ERROR'>(error, {
        isError: isAppError,
        tagError: 'APP_ERROR',
      });
