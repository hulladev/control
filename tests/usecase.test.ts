...
describe('function with only err', () => {
  ...
  it('should return an err type', () => {
    ...
    const r1 = result.match(
      (v) => v,
      (e) => e,
    );
    const r2 = result2.match(
      (v) => v,
      (e) => e,
    );
...
describe('function call with ok and err types', () => {
  ...
  it('should return an err type', () => {
    ...
    ).toStrictEqual(new Error('Value is not positive'));
    const r = result.match(
      (v) => v,
      (e) => e,
    );
  ...
  it('should return an ok type with a promise', () => {
    const result = doSomething(1);
    expect(result.isOk()).toBe(true);
    expect(result.isErr()).toBe(false);
    expect(
  ...
  it('should return an err type with a promise', () => {
    const result = doSomething(0);
    expect(result.isOk()).toBe(false);
    expect(result.isErr()).toBe(true);
    expect(
